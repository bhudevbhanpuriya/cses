...
void solve() {
    ...
    // int maxEnd = v[0][1] , currInd = v[0][2];
    // stack<pair<int,int>> st; // maxEnd , currInd;
    // st.push({maxEnd , currInd});

    vector<int> contains(n) , contained(n);

    // for(int i=1;i<n;i++){
    //     int end = v[i][1], ind = v[i][2];
    //     while(!st.empty() && end > st.top().first){
    //         st.pop();
    //     }
    //     if(!st.empty()){
    //         contains[st.top().second] = 1;
    //         contained[ind] = 1;
    //         st.push({end, ind});
    //     }
    ...
    int mx = -1;
    for(int i = 0; i < n; i++){
        int currEnd = v[i][1] , currInd = v[i][2];
        if(currEnd <= mx) contained[currInd] = 1;
        mx = max(mx, currEnd);
    }
    ...
    for(int i = n - 1; i >= 0; i--){
        int currEnd = v[i][1] , currInd = v[i][2];
        if(currEnd >= mn) contains[currInd] = 1;
        mn = min(mn, currEnd);
    }
    vout(contains);
    vout(contained);
